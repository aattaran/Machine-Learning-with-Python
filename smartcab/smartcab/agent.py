...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            # self.epsilon = 1.0/(self.counter**2 + self.alpha*self.counter)
            # self.epsilon = 1.0/(self.counter**2 - self.alpha*self.counter)
            # self.epsilon = math.fabs(math.cos(self.alpha*self.counter))
            # self.epsilon = math.fabs(math.cos(self.alpha*self.counter))/(self.counter**2)
            # self.epsilon = 1.0/(self.counter**2)
    ...
    def choose_action(self, state):
        ...
        if self.learning and self.epsilon < random.random():
            ...
            #choose the max Q if there are more than one max values
            #valid_actions = [action for action in valid_actions if valid_actions[actions]==max_Q_val]
               
            valid_actions = [action for action in self.Q[state] if max_Q_val==self.Q[state] ]
    ...
    def learn(self, state, action, reward):
        ...
        #v=(1-alpha)v+alpha*a from the lectures
